...
## 4 Modèles additifs généralisés (GAM)
  ...
  Les GAM sont des modèles particulièrement adaptés à la prédiction de la consommation d'électricité. En effet, pour obtenir un fit aussi proche des données que l'on veut il suffit que la consommation s'écrive plus ou moins sous la forme : 
$$ y_i=X_i\beta + f_1(x_{1,i})+ f_2(x_{2,i},x_{3,i})+...+ f_d(x_{d,i})+\epsilon_i \  \text{ avec } \ \epsilon_i \sim \mathcal{N}(0,\,\sigma^{2})$$ 
  Dans notre cas, nous avons la chance d'avoir beaucoup de données et par conséquent le risque d'overfit est assez modéré donc on peut se permettre une telle décomposition qui implique forcément d'avoir un grand nombre de paramètres. Pour autant, dans cette partie nous allons construire un GAM avec pour objectif d'avoir des **f_i** pas trop compliquées. Pour ce faire nous allons dans un premier temps analyser les dépendance en chacune des features. Puis nous améliorerons les performances en faisant un modèle par heure. Nous utilisons des splines de régression cubique pour des raisons que l'on expliquera plus tard.

### Modèles variable par variable, pour optimiser la valeur de k

  On vient de voir les forêts aléatoires qui nous ont données des résultats satisfaisant mais on peut espérer avoir de meilleurs résultats avec des modèles GAM bien paramétrés. Pour ce faire, on va dans un premier temps effectuer un GAM variable par variable pour comprendre leur influence sur la consommation et pour déterminer le degré de liberté optimal pour bien décrire les données sans pour autant faire un modèle trop complexe. L'étude des forêts nous a fourni l'importance des variables ce qui donne une guideline pour choisir lesquelles traiter en premier. Cette partie étant purement explicative, nous ne nous limitons pas à un modèle par heure pour simplifier l’interprétation qui, de toute manière, ne change pas si on observait les 24 résidus à chaque fois.
  
  On commence par la variable qui contient le plus d'information à savoir **Load.48** et on tatonne pour obtenir une bonne valeur de k. Ici ce choix n'était pas très dur à faire car comme on le voit bien sur la figure de droite, la dépendance de **Load** en **Load.48** est quasi linéaire. On choisit donc k le plus petit possible à savoir **k=3**.
...
On obtient une explication de la variance de près de 92% donc l'amélioration est encore notable. Dans le prochain GAM, on ajoute toutes les données discrète à savoir l'heure via **tod** et le jour via **WeekDays**. Et compte-tenu du fait qu'à terme on veut faire un model par heure, on choisit **k=24** pour **tod**
...
Pour ce troisième GAM, on obtient un R2 de plus de 97% et on remarque que même si on laisse un grand degré de liberté pour la dépendance en **toy** (par exemple **k=48**) cela n'améliore pas le R2. Cela justifie le fait de faire un modèle par heure plutôt qu'un par demi-heure. On peut également noter que les Résidus commencent de plus en plus à ressembler à du bruit ce qui est un bon indicateur.

Pour le prochain ajout de feature (**toy** *time of the year*), on décide de ne plus choisir k en tâtonnant car la dépendance est moins facile à voir. On décide donc de trouver la valeur **k** par cross validation. Pour ce faire, on sépare les données en 10 blocks où 9 servent à l'entraînement et le dernier sert à obtenir une valeur du **RMSE**.

```{r , echo=FALSE,fig.align='center'}
load("A_AFFICHER/cross_validation")
plot(K, rmseK, type='b', pch=20,main="",xlab="",ylab="")
points(K[12],rmseK[12],col="red")
title(main="Cross validation pour trouver le k de toy",xlab="dimension de la base de spline (k)",ylab="RMSE (obtenu par block)")
legend(x="topright",legend=c("k=14"),col=c("red"),pch=c(20))
```

On choisit donc k=14 par technique du coude. On réutilisera une cross-validation pour définir la base de spline que l'on va utiliser au final. Mais avant cela, on calcule le GAM final avec les degrés de liberté obtenus dans cette partie (et d'autres obtenus à part pour ne pas que cela soit trop redondant dans le rapport) ce qui donne alors l'equation suivante :
...
Dans cette partie, on va utiliser l'équation obtenue précédemment et analyser les propriétés de généralisation de ce GAM en le testant sur l'année suivante. Mais avant cela, on justifie notre choix de base de spline par cross-validation. 
...
On note que les résultats sont sensiblement les mêmes pour thin plate regression et pour cubic regression et légèrement moins bon pour cyclic cubic regression. On décide donc a posteriori que notre choix de base **cr** est justifié car c'est celle qui donne le meilleur RMSE. Toutefois, on fait remarquer qu'une étude précise nécessiterait de faire ce travail pour trouver la meilleure base de splines adaptée à chaque variables. Cependant nous n'avons pas jugé cela pertinent aux vues des faibles différences de performance. 

Désormais, on peut commencer à analyser les performances de notre GAM dans le cas où on ne fait pas encore un modèle heure par heure. On plot donc les prévisions produites par notre model sur l'année 2019.
...
On voit dans ce plot annuelle que notre prédiction semble globalement correspondre à la consommation réelle, pour autant cette échelle ne permet pas une analyse fine donc on va s'attarder plutôt sur un mois en particulier, on choisit arbitrairement le mois de Décembre en nous disant que c'est un mois intéressant dans le sens où la prévision est près d'un an après la dernière donnée d'entraînement et que la consommation est assez mouvementée en décembre notamment de par les chutes de température.
...
Dans cette partie, on ne se contente plus d'un unique modèle GAM pour faire nos prédictions mais on décide d'en faire un par heure. Ce choix est motivé d'une part par l'importance de la variable **tod** et d'autre part par le fait qu'on imagine qu'il y a de nombreuses intercorrélations entre l'heure et les autres features. Par exemple il est légitime d'imaginer que la **Température** ne va pas avoir le même effet sur la consommation la nuit qu'en journée. C'est donc pour toutes ces raisons que nous avons décidé de construire ces 24 models en réutilisant l'équation obtenu précédemment.
...
Pour des raisons de visibilité, nous affichons l'attache aux données et la prédiction pour le modèle **gam_ouvre_12** et par conséquent nous limitons notre analyse à midi. Dans ces plot, on remarque que l'on fit presque parfaitement les données d'entraînement ce qui pourrait présager d'un potentiel surapprentissage. Pour autant, il n'en est rien puisqu'on voit dans la figure de droite que notre prédiction est elle aussi très proche de la consommation effective. On peut dors et déjà dire que ce découpage par heure est une réussite. 
...
```
On peut tout de même avoir envie de comparer ces performances avec notre précédant GAM. Même si ce n'est pas évident à cette échelle, on remarque tout de même une légère amélioration de la version où on décomposer heure par heure même si la différence est telle qu'il faudrait un indicateur tel que le **MAPE** pour vraiment lequel de ces deux models est meilleur à midi.
...
Comme dans la partie précédante, on s'intéresse aux fonctions des différentes features. On remarque alors que la dépendance est globalement la même qu'avec le GAM sans découpage heure par heure ce qui justifie le fait de ne pas avoir à refaire le travail sur le choix de **k** et de la base de splines.
...
```
Maintenant, il s'agit d'obtenir le **MAPE** en fonction de leur de ces GAMs et de le comparer à des références, ici on prend la prévision *dayahead* de **RTE** ainsi que celle du GAM sans découpage. La première remarque que l'on peut faire est que 
...
Nous obtenons la courbe de MAPE ci-dessous.
...
L'on voit que nous sommes légèrement au-dessus de la prédiction de RTE en termes d'erreur, mais les courbes se croisent en plusieurs points, et particulièrement la nuit ou très tôt le matin, quand la consommation est plus faible.
