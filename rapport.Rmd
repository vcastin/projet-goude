...
Comme nous l'avons souligné, le modèle décrit dans la partie précédente reste une approche assez naïve, car dans notre cas, il est clair que la variance n'est pas la même selon que la consommation est importante ou non. Par conséquent, nous avons choisi d'utiliser des QGAM, suivant un procédé qui s’affranchit du cadre paramétrique dans lequel les GAM sont contraints en utilisant des méthodes de calibrations Bayesiennes -- voir l'article *Bayesian Nonparametric Quantile Regression Modeling in R*, de Fasiolo et al. (2021) -- ce qui permet à la variance de ne plus être une simple constante. Pour estimer un quantile fixé de la loi des données, on entraîne notre modèle avec une fonction de perte classique pour ce genre de problème, la Pinball Loss : 
...
- QGAM aux quantiles 0.4, 0.45, 0.5, 0.55 (partie 7.3)
...
Nous obtenons la figure ci-dessous du **MAPE** de la combinaison convexe obtenue par agrégation en fonction de l'heure, à comparer à celui donné par **RTE dayahead**. La première chose marquante est que nous avons enfin un modèle meilleur que RTE !
...
En effet, nous sommes légèrement en dessous de la prédiction de RTE en termes d'erreur, même si cela se joue à quelques 0.1% près. On remarque également que c'est la nuit que l'écart est le plus important, mais nous manquons d'information sur comment fonctionne le prédicteur de RTE pour pouvoir interpréter cela. Rappelons enfin que le fait que nos performances soient meilleures que celles de RTE est à relativiser, car les données utilisées ne sont pas exactement les mêmes, en particulier pour les données météo. Nous disposons des mesure effectives alors que RTE n'avait à disposition que des prévisions, ce qui augmente l'aléa de leur prédiction.
...
Dans le premier graphique, on remarque que globalement tous nos experts ont du poids à l'exception peut-être de **foret** et du **qgam0.55**. En ce qui concerne la forêt, cela doit venir du fait que c'est assez mauvais prédicteur. Pour le **qgam** de quantile supérieur cela doit être pour la même raison que l'observation que nous avons faites pour les QGAM, c'est-à-dire que 2019 était une année où certains quantiles étaient décalés. On note également que les prédicteurs obtenus par boosting sont ceux qui sont les plus utilisés, ce qui peut s'expliquet par leurs très bonnes performances individuelles. Enfin, nous avons observé une certaine diversité dans le choix des experts en fonction des heures. Pour les données à minuit par exemple, le prédicteur **qgam0.4** a un poids presque égal à 1.
...
Nous avons au travers de ce projet découvert et utilisé de nombreuses méthodes de Machine Learning. La consommation électrique étant un phénomène relativement stable, et nos données de très bonne qualité, toutes ces méthodes ont donné de très bons résultats. Nos plus mauvais prédicteurs ont en effet un MAPE de l'ordre de 3%, ce qui est tout à fait acceptable, tandis que les meilleurs passent presque sous la barre symbolique des 1%.

Un choix crucial a été de séparer nos données en fonction de l'heure, et d'entraîner 24 modèles différents, ce qui a conduit à une amélioration significative des performances de nos modèles. Rappelons également que nous avons supprimé les jours fériés de notre jeu de données. Une piste future serait de prédire spécifiquement la consommation des jours fériés, le principal obstacle étant que l'on dispose de très peu de données.

Nous avons enfin découvert la prévision en loi et les QGAM, qui sont venus enrichir notre palette d'outils de prévision des séries temporelles, et ont donné des résultats satisfaisants, en particulier pour l'agrégation d'experts.
