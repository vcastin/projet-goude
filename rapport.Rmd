...
d <- readRDS("DONNEES/Data_RTE_janv2012_oc2022.RDS")
```
...
plot(mape_we, main = "MAPE en fonction de l'heure, le week-end (année 2019)", xlab = "Heure", ylab = "MAPE (%)", type="l", ylim= c(0,1.8), lab=c(5,4,0), col="blue")
lines(mape_ouvre_RTE, col="red")
legend(x="bottomright", legend=c("foret","RTE"), col=c("blue","red"), pch=c(15,15))
...
plot(d_test_plot$Load, xlab = "Date", ylab = "Consommation (MWh)", type="l", lab=c(5,4,0), col="black") 
lines(d_test_plot$Forecast_RTE_intraday, xlab = "Date", ylab = "Consommation (MWh)", type="l", lab=c(5,4,0), col="blue") 
lines(predictgamdecembre, xlab = "Date", ylab = "Consommation (MWh)", type="l", lab=c(5,4,0), col="red")
legend(x="bottomright", legend=c("Donnée Réel","RTE","GAM"), col=c("black","blue","red"), pch=c(15,15,15))
...
### 6.2 Présentation & une première approche naîve
...
qgam.preds <- sapply(Viz_24, predict, newdata = data_test)
...
lines(data_test$Date, qgam_predicteurs[[18]], col="cyan") 
lines(data_test$Date, gam_normale_predicteurs[[18]], col="blue") 
lines(data_test$Date, qgam.preds1[,"0.9"], col="purple") 
legend("top",legend=c("data test","gam + normale 90%","qgam 90%","mauvais qgam (underfitting)"),col=c("black","cyan", "blue","purple"), lty=1:1:1:1)
...
Cela ne va pas particulièrement dans le sens des observations faites précédemment ce qui confirme le fait que cette mesure est assez mauvais indicateur de la pertinence des quantiles. En effet, le mauvais QGAM à le même nombre de points au dessus que le bon tandis que c'est GAM+normae qui est le plus proche du quantile recherché. Par ailleurs on note que pour les grands quantiles et donc les consommation élevé, nos modèles semblent sous-estimer leur nombre. A l'inverse, pour les faibles quantiles (10%) on trouve une proportion de plus de 15%. On peut chercher une piste pour expliquer cette observation dans un changement de tendance en fonction de l'année invisible en moyenne mais significative pour les valeurs extrêmes.
##Référence

```{r}
d_tot<-rbind (d_ent_ouvre,d_test_ouvre)
plot(d_tot$Date,d_tot$Load,type="l")
```
On Voit effectivement que nous testons nos données sur une année avec assez peu de concentration extremement élevés et à l'inverse plus de consommation très faibles. Pour autant, il n'y a clairement pas de tendance qui semblait annoncer cela et par conséquent on reste sur notre hypothèse selon laquelle l'année n'est pas une feature exploitable. 
